...
public record struct FoundVBlood(PrefabGUID Value);
...
public class FoundVBloodConverter : CommandArgumentConverter<FoundVBlood>
{
	...
	{
		if (!Helper.TryGetPrefabGUIDFromString(input, out PrefabGUID prefabGUID))
		{
			foreach (var kvp in NameToVBloodPrefab)
			{
				if (kvp.Key.ToLower().Replace(" ", "").Contains(input.Replace(" ", "").ToLower()))
				{
					prefabGUID = kvp.Value;
					return new FoundVBlood(prefabGUID);
				}
			}
		}
